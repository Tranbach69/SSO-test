...
namespace MARShop.API.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        [HttpGet("sso/google")]
        //[Authorize]
        public async Task<IActionResult> GoogleSso()
        {
            var user = await HttpContext.AuthenticateAsync(); // Nhận thông tin người dùng từ token
            var userId = user.Principal.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            ...
            return Ok(new { UserId = userId, Name = user.Principal.FindFirst(ClaimTypes.Name)?.Value });
        }
